...
# AICoreDirector - 企业级AI治理与编排平台

中文版 | [English README](README.md)

[![CI](https://github.com/your-org/AICoreDirector/actions/workflows/ci.yml/badge.svg)](.github/workflows/ci.yml)
...
**AICoreDirector** 是一个企业级 **AI治理平台**，统一承载公司所有大模型调用、AI相关能力组件与传统ML模型的服务能力。提供统一接口、智能路由与调度、Fallback机制及集成管理，作为企业级生产需求的 **AI能力总线与中枢**。

### 🚀 **核心价值主张**
- **企业级AI治理**: 集中管理所有AI能力和模型
- **LLM编排平台**: 多模型智能路由和负载均衡
- **AI服务网格**: 统一API网关，治理所有AI/ML服务
- **插件生态**: 可扩展架构，支持自定义AI能力
...
```mermaid
sequenceDiagram
    participant API as "API服务层 (FastAPI)"
    participant Loader as "插件加载器 (core.plugin_loader)"
    participant Biz as "business/*.py"
    participant Router as "FastAPI Router"
    participant Watcher as "热加载线程"

    API->>Loader: startup() 调用 scan_and_register_plugins(app)
    Loader->>Biz: import business.<module> 执行 @plugin_api
    Biz-->>Loader: 在 plugin_registry 注册 {func_name: (func, module)}
    Loader->>Router: add_api_route("/"+func_name, get_plugin_func(...))
    API->>API: move_dynamic_router_to_end()
    API->>Watcher: start_plugin_hot_reload()

    Note over Watcher: 目录变更
    Watcher->>Loader: scan_and_register_plugins(app)
    Loader->>Router: 补注册缺失路由
```
...
sequenceDiagram
    ...
    participant API as API服务层
    participant Registry as plugin_registry
    participant Plugin as 具体插件函数

    alt 直接路由调用
        Client->>API: POST /<plugin_name> {参数}
        API->>Plugin: get_plugin_func 调用（Pydantic解析body）
        Plugin-->>Client: 返回 result
    else 统一入口调用
        Client->>API: POST /plugin/invoke?plugin_name=<name>
        API->>Registry: 查找 plugin_name -> plugin_func
        API->>API: 处理 payload/batch_payload，注入 llm_context
        API->>Plugin: 调用（支持并发/批量）
        API-->>Client: 返回 {"result" | "results"}
    end
```

说明：
- 注册发生在启动与热加载线程触发的扫描阶段，通过 `@plugin_api` + `scan_and_register_plugins(app)` 自动把插件函数注册为一级路由（`/函数名`）。
- 调用在请求阶段发生，既可直达 `/{plugin_name}`，也可经由统一入口 `/plugin/invoke?plugin_name=...`（支持批量与并发）。
